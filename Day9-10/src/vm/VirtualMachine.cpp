...
Step::StackVirtualMachine::operand_type Step::StackVirtualMachine::get_immediate() {
...
void Step::StackVirtualMachine::execute_pop() {
    --_sp;
}

void Step::StackVirtualMachine::execute_iconst(Step::StackVirtualMachine::operand_type operand) {
    *_sp = std::make_shared<LongInteger>(operand);
    ++_sp;
}

void Step::StackVirtualMachine::execute_ipush(Step::StackVirtualMachine::operand_type operand) {
    *_sp = std::make_shared<LongInteger>(operand);
    ++_sp;
}

void Step::StackVirtualMachine::execute_iload(Step::StackVirtualMachine::operand_type operand) {
    *_sp = _constant_pool.at(operand);
    ++_sp;
}

void Step::StackVirtualMachine::execute_istore(Step::StackVirtualMachine::operand_type operand) {
...
void Step::StackVirtualMachine::execute_fconst(Step::StackVirtualMachine::operand_type operand) {
    union {
...
void Step::StackVirtualMachine::execute_fpush(Step::StackVirtualMachine::operand_type operand) {
    union {
        Step::StackVirtualMachine::operand_type _data;
        f64 _d;
    } f = { operand };
...
void Step::StackVirtualMachine::execute_fload(Step::StackVirtualMachine::operand_type operand) {
    *_sp = _constant_pool.at(operand);
    ++_sp;
}

void Step::StackVirtualMachine::execute_fstore(Step::StackVirtualMachine::operand_type operand) {
    union {
        Step::StackVirtualMachine::operand_type _data;
        f64 _d;
    } f = { operand };
...
i32 Step::StackVirtualMachine::print_instruction(u8 opcode, std::size_t index, bool binary) const {
    bool integer = true;
    switch (Step::as<OpCode>(opcode)) {
        ...
        case OpCode::istore:
        {
            ...
            for (i32 i = 0; i < 8; ++i) {
                if (binary) print1byte_bin(t._bytes[7 - i]);
                else print1byte(t._bytes[7 - i]);
            }
            if (!binary) {
                std::cout << "    // " << Step::to_string(to_opcode(opcode)) 
                    << " ";
...
void Step::StackVirtualMachine::print_code() const {
    std::cout << ".code\n";
    int offset = 0;
    std::cout << "offset\topcode byte8 byte7 byte6 byte5 byte4 byte3 byte2 byte1\tinstruction\n";
